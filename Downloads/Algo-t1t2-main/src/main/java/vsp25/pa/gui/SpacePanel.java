...
/**
 * An instance is a JPanel where the game's vsp25.pa.graph is drawn.
 */
...
public class SpacePanel extends JPanel {
    ...
    private ConcurrentMap<Planet, vsp25.pa.gui.Planet> nToP;
    ...
    public void init(Model m) {
        ...
        for (Planet n : m.planets()) {
            vsp25.pa.gui.Planet p = vsp25.pa.gui.Planet.make(n.name(), new Point(n.x(), n.y()), area, bounds,
                    fm, r);
            nToP.put(n, p);
        }
     ...
     */
    public vsp25.pa.gui.Planet getPlanet(Planet n) {
        return nToP.get(n);
    }
    ...
    private void changeBounds() {
        Rectangle2D bounds = getBounds();
        for (vsp25.pa.gui.Planet p : nToP.values()) {
            p.setBounds(bounds);
        }
